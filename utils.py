...
class MultinomialSampling:
    ...
    def __call__(self, output: ModelOutput) -> torch.Tensor:
        ...
        for batch_idx in range(probs.shape[0]):
            output.tokens[batch_idx] = torch.multinomial(
                probs[batch_idx], num_samples=1
            ).squeeze(-1)
        return output
...
class RandomRemaskStrategy:
    ...
    def __init__(
        self, mask_token_id: int, t_min: torch.tensor, mask: torch.Tensor | None = None
    ):
    ...
    def _check_initial_mask(self, outputs: ModelOutput) -> bool:
        if self.mask is None:
            self.mask = torch.ones_like(
                outputs.tokens, device=outputs.tokens.device, dtype=torch.bool
            )
    ...
    def __call__(
        self, x: torch.Tensor, outputs: ModelOutput, t: torch.Tensor
    ) -> torch.Tensor:
...
def build_optimizer_and_scheduler(
    ...
    logger.info(
        f"Scheduler created: total_steps={total_steps}, num_warmup_steps={num_warmup_steps}, warmup_ratio={warmup_ratio}"
    )
    return {
        "optimizer": optimizer,
        "lr_scheduler": {
